...
class FastNoise:
    ...
    def gen_uniform_grid_2d(self, noise_out: List[float], x_start: int, y_start: int, x_size: int, y_size: int, frequency: float, seed: int) -> OutputMinMax:
        # Convert the Python list to a ctypes array of c_float
        noise_out_array = (ctypes.c_float * len(noise_out))(*noise_out)
        min_max = (ctypes.c_float * 2)(0.0, 0.0)  # Initialize min_max as a ctypes array
        self._fn_gen_uniform_grid_2d(self.node_handle, noise_out_array, x_start, y_start, x_size, y_size, frequency, seed, min_max)
        # Convert the ctypes array back to a Python list (if needed)
        noise_out[:] = list(noise_out_array)
        return OutputMinMax(list(min_max))
